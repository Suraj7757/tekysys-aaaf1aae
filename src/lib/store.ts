...
function seedIfEmpty() {
  ...
  const jobs: RepairJob[] = [
    ...
    },
    {
      ...
      deviceBrand: "OnePlus",
      deviceModel: "12",
      problemDescription: "Charging port issue",
      technicianName: "Ravi",
      status: "Ready",
      estimatedCost: 1500,
    ...
    },
    {
      ...
      problemDescription: "Software issue",
      technicianName: "Ravi",
      status: "Received",
      estimatedCost: 800,
...
export const store = {
  ...
  findCustomerByMobile: (mobile: string) =>
    store.getCustomers().find((c) => c.mobile === mobile),
  ...
  clearDeliveredJobs: () => {
    ...
    store.savePayments(
      store.getPayments().filter((p) => !delivered.includes(p.repairJobId)),
    );
  },
  nextJobId: () => {
    const counter =
      parseInt(localStorage.getItem(STORAGE_KEYS.jobCounter) || "0") + 1;
  ...
  addSettlement: (s: SettlementCycle) => {
    const all = store.getSettlements();
    all.push(s);
    store.saveSettlements(all);
  },

  getInventory: () => get<InventoryItem>(STORAGE_KEYS.inventory, []),
  saveInventory: (i: InventoryItem[]) => set(STORAGE_KEYS.inventory, i),
  addInventoryItem: (i: InventoryItem) => {
