...
export function generateInvoicePDF(job: RepairJob, payment: Payment | undefined, settings: ShopSettings) {
  ...
  doc.text(`Date: ${new Date(job.createdAt).toLocaleDateString()}`, 75, currentY, { align: 'right' });
  currentY += 5;
  ...
  const tableData: any[][] = [];
  tableData.push(['Service', job.problemDescription, `Rs.${job.estimatedCost}`]);
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.text(`Total Amount: Rs.${payment ? payment.amount : job.estimatedCost}`, 75, currentY, { align: 'right' });
  currentY += 6;
  ...
  currentY += 3;
  doc.text(`Visit again to ${settings.shopName || 'RepairXpert'}`, 40, currentY, { align: 'center' });
...
export function generateSellInvoicePDF(sell: SellInvoiceData, settings: ShopSettings) {
  ...
  doc.text(`Date: ${new Date(sell.createdAt).toLocaleDateString()}`, 75, currentY, { align: 'right' });
  currentY += 5;
  ...
  autoTable(doc, {
    ...
    body: [[sell.itemName, String(sell.quantity), `Rs.${sell.sellPrice}`, `Rs.${sell.total}`]],
    theme: 'plain',
    styles: { fontSize: 7, cellPadding: 1 },
    headStyles: { fontStyle: 'bold', lineColor: [0, 0, 0], lineWidth: 0.1 },
  });
