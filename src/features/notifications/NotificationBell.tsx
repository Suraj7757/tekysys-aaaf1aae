    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {unread > 0 && (
            <span
              className="absolute -top-0.5 -right-0.5 h-4.5 w-4.5 rounded-full bg-red-500 text-white text-[9px] font-black flex items-center justify-center animate-pulse"
              style={{ height: "1.1rem", width: "1.1rem" }}
            >
              {unread > 9 ? "9+" : unread}
            </span>
      ...
      <PopoverContent
        align="end"
        className="w-80 p-0 shadow-xl rounded-2xl border overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-violet-600 to-indigo-600">
          <div className="flex items-center gap-2 text-white">
            ...
            {unread > 0 && (
              <Badge className="bg-white/30 text-white border-0 text-[10px] h-4 px-1">
                {unread}
              </Badge>
            )}
          </div>
          {unread > 0 && (
            <button
              onClick={markAllRead}
              className="text-violet-200 hover:text-white text-xs flex items-center gap-1 transition-colors"
            >
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((n) => {
              ...
              return (
                ...
                >
                  <div
                    className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${cfg.bg}`}
                  >
                    <Icon className={`h-4 w-4 ${cfg.color}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-xs font-bold ${!n.read ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {n.title}
                    </p>
