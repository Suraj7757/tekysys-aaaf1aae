...
export default function TrackOrder({ isModal = false }: { isModal?: boolean }) {
  ...
  const handleTrack = () => { if (trackingId.trim()) handleTrackDirect(trackingId.trim()); };
  ...
  const downloadInvoicePDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Date: ${new Date(result.created_at).toLocaleDateString('en-IN')}`, 14, 52);
    doc.text(`Status: ${result.status}`, 14, 59);

    if (result.type === 'job') {
      doc.text(`Customer: ${result.customer_name}`, 14, 66);
      doc.text(`Device: ${result.device_brand} ${result.device_model || ''}`, 14, 73);
      autoTable(doc, {
        startY: 82,
  ...
  const isDelivered = result?.status === 'Delivered' || result?.status === 'Completed';
  const isRejected = result?.status === 'Rejected' || result?.status === 'Unrepairable' || result?.status === 'Returned';
  const amount = result?.type === 'job' ? result?.estimated_cost : result?.total;

  return (
    <div className={`${isModal ? 'p-4' : 'min-h-screen p-4'} bg-gradient-to-br from-slate-50 via-white to-violet-50 dark:from-slate-950 dark:via-slate-900 dark:to-violet-950`}>
      {!isModal && (
        <Link to="/" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-primary transition-colors mb-6">
          <ArrowLeft className="h-4 w-4" /> Back to Home
        </Link>
      )}

      <div className={`w-full max-w-lg mx-auto space-y-5 ${!isModal ? 'pt-4' : ''}`}>
        {/* Header */}
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Enter your Job or Sell tracking ID below</p>
        </div>
        ...
        {!loading && searched && !result && (
          <Card className="border-red-200 bg-red-50 dark:bg-red-950/20">
            <CardContent className="p-8 text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">No order with this ID was found. Check and try again.</p>
              <p className="text-xs text-muted-foreground">Jobs: trackable 3 days after delivery • Sales: trackable 30 days</p>
            </CardContent>
          </Card>
        )}
        ...
        {!loading && result && (
          <div className="space-y-4">
            ...
            <Card className="border-0 shadow-xl overflow-hidden">
              <div className={`p-4 ${isRejected ? 'bg-gradient-to-r from-red-500 to-orange-500' : isDelivered ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 'bg-gradient-to-r from-violet-600 to-indigo-600'} text-white`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs opacity-75 font-medium">Tracking ID</p>
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {result.type === 'job' ? (
                    <>
                      ...
                      <InfoBox label="Device" value={`${result.device_brand} ${result.device_model || ''}`} />
                      <InfoBox label="Problem" value={result.problem} />
                      <InfoBox label="Amount" value={`₹${Number(result.estimated_cost).toLocaleString()}`} highlight />
                      <InfoBox label="Created" value={new Date(result.created_at).toLocaleDateString('en-IN')} />
                      {result.delivered_at && <InfoBox label="Delivered" value={new Date(result.delivered_at).toLocaleDateString('en-IN')} />}
                    </>
                  ) : (
                    <>
                      <InfoBox label="Item" value={result.item_name} />
                      <InfoBox label="Quantity" value={String(result.quantity)} />
                      <InfoBox label="Total" value={`₹${Number(result.total).toLocaleString()}`} highlight />
                      <InfoBox label="Date" value={new Date(result.created_at).toLocaleDateString('en-IN')} />
                    </>
                  )}
                </div>
                ...
                {result.type === 'job' && !isRejected && (
                  <div className="mt-4 pt-4 border-t">
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">Repair Progress</p>
                    <div className="relative">
                      ...
                      <div className="space-y-4">
                        {JOB_STATUSES.map((step, i) => {
                          ...
                          return (
                            <div key={step.key} className="flex items-center gap-3 relative pl-2">
                              <div className={`h-8 w-8 rounded-full flex items-center justify-center z-10 transition-all ${
                                isDone ? 'bg-gradient-to-br from-violet-600 to-indigo-600 shadow-md shadow-violet-500/30' : 'bg-muted border-2 border-border'
                              }`}>
                                {isDone ? (
                                  i < currentStep ? <CheckCircle2 className="h-4 w-4 text-white" /> : <Icon className="h-4 w-4 text-white" />
                                ) : (
                                  <Icon className="h-4 w-4 text-muted-foreground" />
                                )}
                              </div>
                              <div className="flex-1">
                                <p className={`text-sm font-bold ${isCurrent ? 'text-primary' : isDone ? 'text-foreground' : 'text-muted-foreground'}`}>
                                  {step.label}
                                  {isCurrent && <span className="ml-2 text-[10px] bg-primary text-primary-foreground px-2 py-0.5 rounded-full">Current</span>}
                                </p>
                              </div>
                            </div>
                          );
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t">
                  <Button variant="outline" size="sm" className="w-full gap-1.5 text-xs" onClick={downloadInvoicePDF}>
                    <FileText className="h-3.5 w-3.5" /> Download Invoice
                  </Button>
                  {(isDelivered || isRejected) ? (
                    <Button variant="outline" size="sm" className="w-full gap-1.5 text-xs" onClick={() => setShowFeedback(!showFeedback)}>
            ...
            <div className="flex flex-col items-center gap-4 py-2">
              ...
              <div className="text-center">
                ...
                <div className="flex items-center gap-4 justify-center">
                  <div className="flex flex-col items-center gap-1">
                    <img src="https://img.icons8.com/color/48/google-pay-india.png" className="h-7 w-7" alt="GPay" />
                    <span className="text-[8px] font-bold text-muted-foreground uppercase">GPay</span>
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <img src="https://img.icons8.com/color/48/phonepe.png" className="h-7 w-7" alt="PhonePe" />
                    <span className="text-[8px] font-bold text-muted-foreground uppercase">PhonePe</span>
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <img src="https://img.icons8.com/color/48/paytm.png" className="h-7 w-7" alt="Paytm" />
                    <span className="text-[8px] font-bold text-muted-foreground uppercase">Paytm</span>
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <img src="https://img.icons8.com/color/48/bhim.png" className="h-7 w-7" alt="BHIM" />
                    <span className="text-[8px] font-bold text-muted-foreground uppercase">BHIM</span>
                  </div>
                </div>
              </div>
              ...
              <Button 
                ...
                onClick={() => window.location.href = `upi://pay?pa=${merchantSettings.upi_id}&pn=${encodeURIComponent(merchantSettings.shop_name || 'Merchant')}&am=${amount}&cu=INR`}
              >
                <Zap className="h-4 w-4" /> Open UPI App
              </Button>
            </div>
                  <div className="space-y-2">
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Submit UTR / Transaction ID</p>
                    <div className="flex gap-2">
                      <Input value={utr} onChange={e => setUtr(e.target.value)} placeholder="12-digit UTR number" className="h-10 font-mono" />
                      <Button
                        onClick={async () => {
                          if (!utr.trim()) { toast.error('Enter UTR number'); return; }
                          setSubmittingPay(true);
                          const { error } = await (supabase as any).from('customer_payments').insert({
                            user_id: result.user_id, tracking_id: result.tracking_id,
                            amount, utr_number: utr, customer_name: result.customer_name || 'Guest', status: 'pending'
                          });
                          if (error) toast.error('Failed to submit. Try again.');
                          else { toast.success('Payment submitted! Shop will verify soon.'); setPayOpen(false); setUtr(''); }
                    ...
                    <p className="text-[10px] text-center text-muted-foreground">Shop owner will verify and update your status.</p>
                  </div>
                </CardContent>
              </Card>
            )}
              ...
              <Card className="border-amber-200 shadow-lg animate-in slide-in-from-top-4 duration-300">
                <CardContent className="p-5 space-y-4">
                  {feedbackSubmitted ? (
                    ...
                    <>
                      <p className="text-sm font-bold flex items-center gap-2"><MessageSquare className="h-4 w-4 text-amber-500" />Rate Your Experience</p>
                      <div className="flex justify-center gap-2">
                        {[1, 2, 3, 4, 5].map(s => (
                          ...
                          >
                            <Star className={`h-8 w-8 ${(hoverRating || rating) >= s ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground'}`} />
                          </button>
                        ))}
                      </div>
                      <Textarea
                      ...
                      />
                      <Button className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white" onClick={submitFeedback}>
                        Submit Review
                      </Button>
                    </>
                  )}
        ...
        <div className="text-center pb-8">
          <Link to="/auth" className="text-sm font-bold text-muted-foreground hover:text-primary transition-all flex items-center justify-center gap-2">
...
function InfoBox({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-xl p-2.5 ${highlight ? 'bg-violet-50 dark:bg-violet-950/30 border border-violet-200' : 'bg-muted/50'}`}>
      <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{label}</p>
      <p className={`text-sm font-bold mt-0.5 ${highlight ? 'text-violet-700 dark:text-violet-400' : 'text-foreground'}`}>{value || '—'}</p>
    </div>
  );
}
