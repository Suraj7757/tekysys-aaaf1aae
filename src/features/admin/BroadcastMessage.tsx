...
import { MessageCircle, Smartphone, Users, Send, CheckCircle, Megaphone } from 'lucide-react';
...
export default function BroadcastMessage() {
  ...
  const phonedCustomers = customers.filter((c: any) => c.phone && c.phone.trim().length >= 10);
  ...
  const handleBroadcast = async () => {
    ...
    if (phonedCustomers.length === 0) { toast.error('No customers with phone numbers'); return; }
    setSending(true);
    setSentCount(0);
    ...
    if (channel === 'whatsapp') {
      ...
      toast.info('WhatsApp opened for first customer. For bulk: use WhatsApp Business API.');
      count = 1;
    } else {
      // SMS: open SMS app
      const phones = phonedCustomers.slice(0, 5).map((c: any) => c.phone.replace(/\D/g, '')).join(',');
      ...
      <Card className="overflow-hidden shadow-card">
        <div className="bg-gradient-to-r from-violet-600 to-indigo-600 p-4 text-white">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-violet-200">Send WhatsApp / SMS to all customers</p>
            </div>
          </div>
          <div className="mt-3 flex items-center gap-3">
            <div className="bg-white/20 rounded-xl px-3 py-1.5 text-sm flex items-center gap-1.5">
              ...
              <span className="text-violet-200 text-xs">customers with phone</span>
            </div>
          </div>
        </div>
        ...
        <CardContent className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Send Via</Label>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className={`h-8 w-8 rounded-full flex items-center justify-center ${channel === 'whatsapp' ? 'bg-[#25D366]' : 'bg-muted'}`}>
                  <MessageCircle className={`h-4 w-4 ${channel === 'whatsapp' ? 'text-white' : 'text-muted-foreground'}`} />
                </div>
                <div className="text-left">
                  <p className={`text-sm font-bold ${channel === 'whatsapp' ? 'text-[#25D366]' : ''}`}>WhatsApp</p>
                  <p className="text-[10px] text-muted-foreground">Best engagement</p>
                </div>
              </button>
              <button
              ...
              >
                <div className={`h-8 w-8 rounded-full flex items-center justify-center ${channel === 'sms' ? 'bg-blue-500' : 'bg-muted'}`}>
                  <Smartphone className={`h-4 w-4 ${channel === 'sms' ? 'text-white' : 'text-muted-foreground'}`} />
                </div>
                <div className="text-left">
                  <p className={`text-sm font-bold ${channel === 'sms' ? 'text-blue-600' : ''}`}>SMS</p>
                  <p className="text-[10px] text-muted-foreground">All phones</p>
                </div>
              </button>
            </div>
          </div>
          ...
          {/* Templates */}
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Message Template</Label>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-muted-foreground">{message.length}/1000 chars</span>
            </div>
            <Textarea
              value={message}
          ...
          {message && (
            <div className="bg-[#ECE5DD] dark:bg-slate-800 rounded-2xl p-3 space-y-1">
              ...
              <div className="bg-white dark:bg-slate-700 rounded-xl rounded-tl-none p-3 shadow-sm max-w-[85%]">
                ...
                <p className="text-[9px] text-right text-muted-foreground mt-1">Now ✓✓</p>
              </div>
            </div>
          )}
          ...
          {doneVisible ? (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-600">Message delivered to {sentCount} customer(s)</p>
              </div>
              <Button size="sm" variant="outline" className="ml-auto" onClick={() => { setDoneVisible(false); setMessage(''); }}>
                New Message
              </Button>
            </div>
          ) : (
            ...
            >
              ...
              {sending ? 'Sending...' : `Send to ${phonedCustomers.length} Customers via ${channel === 'whatsapp' ? 'WhatsApp' : 'SMS'}`}
            </Button>
          )}
        </CardContent>
      </Card>
