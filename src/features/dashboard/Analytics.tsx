...
export default function Analytics() {
  ...
  const jobsInRange = useMemo(
    () => jobs.filter((j) => new Date(j.created_at).getTime() >= cutoff),
    [jobs, cutoff],
  );
  const sellsInRange = useMemo(
    () => sells.filter((s) => new Date(s.created_at).getTime() >= cutoff),
    [sells, cutoff],
  );
  ...
  const revenueTrend = useMemo(() => {
    const map = new Map<
      string,
      { date: string; jobs: number; sells: number; total: number }
    >();
    ...
    jobsInRange.forEach((j) => {
      ...
      const e = map.get(k);
      if (e) {
        e.jobs += Number(j.estimated_cost) || 0;
        e.total += Number(j.estimated_cost) || 0;
      }
    });
  ...
  const topDevices = useMemo(() => {
    ...
    jobsInRange.forEach((j) =>
      m.set(
        j.device_brand || "Unknown",
        (m.get(j.device_brand || "Unknown") || 0) + 1,
      ),
    );
  ...
  const topIssues = useMemo(() => {
    const m = new Map<string, number>();
    jobsInRange.forEach((j) => {
      const k = (j.problem_description || "Other")
        .split(" ")
        .slice(0, 3)
        .join(" ");
      m.set(k, (m.get(k) || 0) + 1);
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Deep analytics over last {days} days
            </p>
          </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Users className="h-4 w-4" /> Top Technicians
              </CardTitle>
            </CardHeader>
            <CardContent>
              {loading ? (
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                ...
                {topCustomers.map((c, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="h-8 w-8 rounded-full bg-gradient-to-br from-primary to-primary/60 text-primary-foreground flex items-center justify-center text-xs font-bold">
                        {i + 1}
                      </span>
                      <div>
    ...
    <Card className={accent ? "border-destructive/40" : ""}>
      <CardContent className="p-4">
        ...
        {loading ? (
          ...
          <p
            className={`text-xl font-bold ${accent ? "text-destructive" : ""}`}
          >
            {value}
          </p>
        )}
