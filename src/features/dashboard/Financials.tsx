...
import { BarChart3, TrendingUp, TrendingDown, Wallet, IndianRupee, Download, Calendar, Filter } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 font-medium">Detailed insights into your shop's revenue, expenses and net profit.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card className="border-0 shadow-xl bg-card overflow-hidden group">
            <CardContent className="p-6 relative">
              ...
              <h3 className="text-3xl font-black text-foreground">₹{totalExpenses.toLocaleString()}</h3>
          ...
          <Card className="border-0 shadow-xl bg-primary overflow-hidden relative group">
            <CardContent className="p-6">
              ...
              <p className="text-xs font-bold text-primary-foreground/70 uppercase tracking-widest mb-1">Net Profit</p>
              <h3 className="text-3xl font-black text-primary-foreground">₹{netProfit.toLocaleString()}</h3>
          ...
          <Card className="border-0 shadow-xl bg-card overflow-hidden group">
            <CardContent className="p-6 relative">
              ...
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Avg. Ticket Size</p>
              <h3 className="text-3xl font-black text-foreground">₹{(totalRevenue / (payments?.length || 1)).toFixed(0)}</h3>
              <p className="mt-4 text-xs text-muted-foreground font-medium">Based on {payments?.length || 0} transactions</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <Card className="lg:col-span-2 border-0 shadow-xl bg-card overflow-hidden">
            <CardHeader>
              <CardTitle className="text-xl font-black flex items-center gap-2">
                <TrendingUp className="h-5 w-5 text-primary" /> Revenue vs Expenses
              </CardTitle>
              <CardDescription>Visualizing your daily cash flow performance.</CardDescription>
            </CardHeader>
            <CardContent className="h-[300px] w-full pt-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{fontSize: 12, fontWeight: 600}} tickFormatter={(value) => `₹${value}`} />
                  <Tooltip 
          ...
          <Card className="border-0 shadow-xl bg-card overflow-hidden">
            <CardHeader>
              <CardTitle className="text-xl font-black">Expense Breakdown</CardTitle>
              <CardDescription>Where your money is going.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 pt-4">
               {[
                 { label: 'Part Purchases', amount: totalExpenses * 0.6, color: 'bg-blue-500' },
                 { label: 'Shop Rent', amount: totalExpenses * 0.2, color: 'bg-amber-500' },
                 { label: 'Staff Salaries', amount: totalExpenses * 0.15, color: 'bg-emerald-500' },
                 { label: 'Miscellaneous', amount: totalExpenses * 0.05, color: 'bg-purple-500' },
               ].map((item, i) => (
                 <div key={i} className="space-y-2">
                   <div className="flex justify-between items-end">
                     <p className="text-sm font-bold text-foreground">{item.label}</p>
                     <p className="text-xs font-black text-muted-foreground">₹{item.amount.toLocaleString()}</p>
                   </div>
                   <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                     <motion.div 
                       initial={{ width: 0 }}
                       animate={{ width: `${(item.amount / (totalExpenses || 1)) * 100}%` }}
        ...
        <Card className="shadow-xl ring-1 ring-white/5 overflow-hidden border-0">
          <CardHeader className="bg-card/50 border-b pb-4 flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-xl font-black tracking-tight">Recent Financial Activity</CardTitle>
              <CardDescription>A consolidated list of all revenue and expense entries.</CardDescription>
            </div>
              ...
              <TableBody>
                {payments?.slice(0, 5).map((p: any) => (
                  <TableRow key={p.id} className="hover:bg-muted/30 transition-colors">
                    <TableCell className="font-bold">Job Payment #{p.job_id}</TableCell>
                    <TableCell className="text-xs text-muted-foreground font-medium">{new Date(p.created_at).toLocaleDateString()}</TableCell>
                    <TableCell><Badge variant="outline" className="text-emerald-600 bg-emerald-500/5 border-emerald-600/20">Revenue</Badge></TableCell>
                    <TableCell className="text-xs font-bold text-muted-foreground">{p.method}</TableCell>
                ...
                {expenses?.slice(0, 5).map((e: any) => (
                  <TableRow key={e.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="text-xs text-muted-foreground font-medium">{new Date(e.date).toLocaleDateString()}</TableCell>
                    <TableCell><Badge variant="outline" className="text-destructive bg-destructive/5 border-destructive/20">Expense</Badge></TableCell>
                    <TableCell className="text-xs font-bold text-muted-foreground">Cash</TableCell>
                    <TableCell className="text-right font-black text-destructive">-₹{e.amount}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
