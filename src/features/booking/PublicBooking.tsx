...
export default function PublicBooking() {
  ...
  const [form, setForm] = useState({
    ...
    device_brand: '', device_model: '', problem_description: '', preferred_date: '',
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-primary/5 to-background">
        <Card className="max-w-md w-full"><CardContent className="p-10 text-center space-y-4">
          ...
          <p className="text-muted-foreground">{shop.shop_name} will contact you shortly on {form.customer_mobile}.</p>
          <Button onClick={() => { setSubmitted(false); setForm({ customer_name: '', customer_mobile: '', customer_email: '', device_brand: '', device_model: '', problem_description: '', preferred_date: '' }); }}>Submit Another</Button>
        </CardContent></Card>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 to-background p-4 md:p-8">
      <div className="max-w-xl mx-auto">
        ...
        <Card>
          <CardHeader><CardTitle>Book a Repair</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={submit} className="space-y-3">
              ...
              <div><Label>Mobile Number *</Label><Input value={form.customer_mobile} onChange={e => setForm({ ...form, customer_mobile: e.target.value })} required /></div>
              <div><Label>Email (optional)</Label><Input type="email" value={form.customer_email} onChange={e => setForm({ ...form, customer_email: e.target.value })} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Device Brand *</Label><Input value={form.device_brand} onChange={e => setForm({ ...form, device_brand: e.target.value })} placeholder="Samsung" required /></div>
                <div><Label>Model</Label><Input value={form.device_model} onChange={e => setForm({ ...form, device_model: e.target.value })} placeholder="Galaxy A50" /></div>
              </div>
              <div><Label>Describe the problem *</Label><Textarea value={form.problem_description} onChange={e => setForm({ ...form, problem_description: e.target.value })} rows={3} required /></div>
              <div><Label>Preferred date (optional)</Label><Input type="date" value={form.preferred_date} onChange={e => setForm({ ...form, preferred_date: e.target.value })} min={new Date().toISOString().slice(0, 10)} /></div>
