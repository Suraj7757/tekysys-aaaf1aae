...
export default function WholesaleDashboard() {
  ...
  const load = async () => {
    ...
    const [itemsRes, ordersRes] = await Promise.all([
      (supabase as any).from('wholesale_catalog').select('*').eq('user_id', user!.id).order('created_at', { ascending: false }),
      (supabase as any).from('customer_orders').select('*').eq('shopkeeper_id', user!.id).order('created_at', { ascending: false }),
    ]);
  ...
  const submit = async () => {
    ...
    const { error } = editing
      ? await (supabase as any).from('wholesale_catalog').update(payload).eq('id', editing.id)
      : await (supabase as any).from('wholesale_catalog').insert(payload);
  ...
  const del = async (id: string) => {
    ...
    const { error } = await (supabase as any).from('wholesale_catalog').delete().eq('id', id);
    if (error) return toast.error(error.message);
    load();
  };

  const updateOrder = async (id: string, status: string) => {
    const { error } = await (supabase as any).from('customer_orders').update({ status }).eq('id', id);
    if (error) return toast.error(error.message);
    load();
  };
